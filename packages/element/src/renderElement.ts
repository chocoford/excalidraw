...
const drawFilteredImageOnCanvas = (
  ...
  tempCanvas.height = Math.max(1, Math.ceil(element.height * devicePixelRatio));
...
const drawElementFromCanvas = (
  ...
  // Match the document that generated the cached bitmap, which can differ
  // from the destination canvas document.
  const devicePixelRatio =
    elementWithCanvas.canvas!.ownerDocument.defaultView?.devicePixelRatio || 1;
  const padding = getCanvasPadding(element);
