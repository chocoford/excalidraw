...
export class AppViewport {
  ...
  getOffsets = (opts?: ViewportOffsetsOptions): Offsets => {
    ...
    const nativeTop = Math.max(0, Number(nativeViewportInsets.top) || 0);
    const nativeRight = Math.max(0, Number(nativeViewportInsets.right) || 0);
    const nativeBottom = Math.max(0, Number(nativeViewportInsets.bottom) || 0);
    const nativeLeft = Math.max(0, Number(nativeViewportInsets.left) || 0);
    ...
    measuredOffsets.top = Math.max(measuredOffsets.top, nativeTop);
    measuredOffsets.right = Math.max(measuredOffsets.right, nativeRight);
    measuredOffsets.bottom = Math.max(measuredOffsets.bottom, nativeBottom);
    measuredOffsets.left = Math.max(measuredOffsets.left, nativeLeft);
