...
describe("UserList", () => {
  it("retains the local dropdown when the same account has multiple clients", async () => {
    ...
    await waitFor(() => {
      ...
      // ExcalidrawZ keeps the default top-right chrome mounted but hidden so
      // the native host owns that surface. Verify the local controls survive
      // collaborator deduplication without requiring the host-hidden wrapper
      // to be visible.
      expect(getByText(document.body, "Spotlight me")).toBeInTheDocument();
    });
  });
});
